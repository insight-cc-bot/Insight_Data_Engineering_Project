...
cluster_conf = {
    ...
    "postgres_server":"spark://ec2-34-211-145-63.us-west-2.compute.amazonaws.com:7077",
    "table_name": "wordcount_table",    
}
...
spark_frequent_words_count = "spark-submit --master {spark} --jars {jars} {prog} {arg1} {arg2}".format(
    spark=cluster_conf["spark_master"],
    jars=cluster_conf["spark_jars"], prog=cluster_conf["spark_prog"], arg1=YEAR, arg2=MONTH)

spark_load_postgres = "spark-submit --master {spark} --jars {jars} {prog} {arg1} {arg2}".format(
    spark=cluster_conf["spark_master"],
    jars=cluster_conf["spark_jars"], prog=cluster_conf["spark_prog"], arg1=YEAR, arg2=MONTH)
...
dag = DAG(dag_id='spark_frequent_words', description='Get frequent words for the corpus', default_args=default_args, schedule_interval=@monthly)

# check if file exist in S3

# read run spark job to calculate words and upload to S3 load to postgres
get_words = BashOperator(
    task_id='run_word_count_job',
    bash_command=spark_frequent_words_count,
    dag = dag)

# set tasks
load_postgres = BashOperator(
    task_id='load_words_to_postgres',
    python_callable=words_to_postgres,
    dag=dag)

# setting dependencies
get_words.set_upstream(load_postgres)